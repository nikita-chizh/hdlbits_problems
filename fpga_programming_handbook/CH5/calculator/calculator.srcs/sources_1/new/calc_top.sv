...
`endif
`timescale 1ns/10ps
module calculator_top #(parameter BITS = 32, parameter NUM_SEGMENTS = `NUM_SEGMENTS)(
    input wire                      clk,
    input wire [15:0]               SW,
    ...
    sys_pll u_sys_pll( .clk_in1 (clk), .reset(0), .clk_out1 (clk_50), .locked (int_reset));
    ...
    calculator_moore#( .BITS(BITS)) u_sm( .clk(clk_50), .reset(reset),
                        .start(button_down), .operation(button_capt),
                        .operand(sw_capt), .accum(accumulator));
